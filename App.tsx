...
import { analyzeReputation, analyzeRiskProfile } from './client-services/geminiService';
import { shortenAddress } from './client-services/walletService';
import { PersistenceService } from './client-services/persistence';
import { AuthService } from './client-services/netlifyAuth';
import { SecurityService } from './client-services/security';
import { ContractService } from './client-services/contractService'; // Import ContractService
  ...
  const handleFundRequest = async (req: LoanRequest) => {
    ...
    try {
      ...
      const updatedReq = {
        ...
        smartContractAddress: receipt.to || '',
  ...
  if (user) {
    ...
    return (
      <div className="flex h-screen bg-[#050505] text-zinc-200 font-sans selection:bg-[#00e599] selection:text-black overflow-hidden relative">
        ...
        {showTutorial && <DemoTutorial onClose={() => setShowTutorial(false)} />}
        ...
        <aside className="w-64 bg-[#0a0a0a] border-r border-zinc-900 flex flex-col z-50">
          ...
          <nav className="flex-1 px-4 space-y-2 mt-4">
            ...
            <button
              ...
              className="flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-all bg-gradient-to-r from-emerald-600 to-teal-600 text-white hover:from-emerald-500 hover:to-teal-500 mt-4"
            >
