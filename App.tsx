...
const App: React.FC = () => {
  ...
  const handleFundRequest = async (req: LoanRequest) => {
    if (!wallet.isConnected) { setShowWalletModal(true); return; }
    if (!user) return;

    // TRIGGER SMART CONTRACT
    const amountEth = (req.amount / 2000).toFixed(4); // Rough conversion for demo: $2000 = 1 ETH
    alert(`Initiating On-Chain Transaction...\nFunding ${amountEth} ETH for Loan ${req.id}`);

    const result = await BlockchainService.fundLoan(req.id, amountEth);

    if (result.success) {
      const updatedReq = {
        ...req,
        status: 'ESCROW_LOCKED' as const,
        smartContractAddress: "0x5FbDB2315678afecb367f032d93F642f64180aa3", // Mock/Real address
  ...
  const handleRepayLoan = async (req: LoanRequest) => {
    ...
    // TRIGGER SMART CONTRACT
    const amountEth = (req.amount / 2000).toFixed(4);
    const interestRate = req.maxInterestRate;

    alert(`Initiating On-Chain Repayment...`);

    const result = await BlockchainService.repayLoan(req.id, amountEth, interestRate);
